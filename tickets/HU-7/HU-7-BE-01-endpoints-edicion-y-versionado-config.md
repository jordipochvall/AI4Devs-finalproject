...
- `GET /api/v1/math/configs/{configId}` — detalle de una versión (`config` completo del apartado 3.3 + RTP/volatilidad **objetivo declarados**).
...
- **AC3**: Un `config` inválido (símbolo inexistente, payline fuera de rango, `triggerSymbol` no `SCATTER`, **hueco de pago** —un recuento alcanzable sin valor en `paytable`/`scatterPays`/`award`, ver cobertura sin huecos en 3.3.3—, etc.) → `422` con `errors[]` por campo.
